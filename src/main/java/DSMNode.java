...
public class DSMNode {
    ...
    private static final long REPLICATION_DELAY_MS = 0; // add 100 milliseconds delay for stressing
    ...
    private final AtomicLong latestSequenceNumber = new AtomicLong(0); 
    private final Map<Long, Runnable> pendingReads = new ConcurrentHashMap<>(); 
    ...
    private void handleWrite(DSMMessage msg) {
        ...
        if (isPrimary) {
            ...
            pendingReplications.put(key, replicaNodes.size()); 

            if (msg.getReplyToQueue() != null) {
                pendingClientReplies.put(key, msg.getReplyToQueue()); 
            }
    ...
    private void decrementPendingReplications(Address address, long sequenceNumber) {
        ...
        synchronized (pendingReplications) {
            ...
            if (remaining <= 0) {
                ...
                String replyQueue = pendingClientReplies.remove(key); 
                if (replyQueue != null) {
                    try {
